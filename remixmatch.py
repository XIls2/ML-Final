...
def main():
   ...
   # labeled_dataset, unlabeled_dataset, test_dataset = DATASET_GETTERS[args.dataset](
    ...
    train_labeled_set, train_unlabeled_set, val_set, test_set = dataset.get_cifar10('./data', args.num_labeled, transform_train=transform_train, transform_val=transform_val)
    ...
    for epoch in range(490, args.epochs):
...
def train(args, labeled_trainloader, unlabeled_trainloader,
    ...
    for batch_idx, (data_x, data_u) in enumerate(train_loader):                     #data_x : labeled data , data_u : unlabeled data
        ...
        #loss_rot=0
        logits_m_x = logits_m_x.float()
        Lx = criterion(logits_m_x, targets_x.to(args.device).long())
        ...
        if not args.no_progress:
            ...
            '''
            p_bar.suffix  = 'Train Epoch: {epoch}/{epochs:4}|Iter: {batch:4}/{iter:4}|LR: {lr:.6f}|Data: {data:.3f}s|Batch: {bt:.3f}s | Loss: {loss:.4f} |Loss_x: {loss_x:.4f}| Loss_u: {loss_u:.4f}| Loss_us: {loss_us:.4f}| Loss_ua: {loss_ua:.4f}| Loss_r: {loss_r:.4f}'.format(
                epoch=epoch + 1,
                epochs=args.epochs,
                batch=batch_idx + 1,
                iter=args.iteration,
